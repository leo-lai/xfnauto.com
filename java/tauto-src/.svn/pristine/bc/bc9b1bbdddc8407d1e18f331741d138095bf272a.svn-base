...
//		int sAccount = account.length()-total;//屏蔽成*的个数
	...
	public static String getSha1Str(File file){
		...
		try{
			...
			str=Base64Util.encodeData(md.digest(baos.toByteArray()));
			fis.close();
		}
		catch(Exception ex){
			ex.printStackTrace();
	...
	public static String getSha1Str(byte[] bts){
		...
		try{
			MessageDigest md = MessageDigest.getInstance("SHA-1");
			str=Base64Util.encodeData(md.digest(bts));
		}
		catch(Exception ex){
			ex.printStackTrace();
		}
...
//    		System.out.println(blank+";"+sign);
    ...
    public static String toJsonResult(BaseRsp rsp){
		try{
			...
			StringBuilder sb = new StringBuilder();
			map.put("key", AppConstants.APPKEY);
			...
			String blank = sb.toString();
			String sign = md5(blank.getBytes(AppConstants.CHARSET));
			System.out.println(blank+";"+sign);
			map.put("sign", sign);
